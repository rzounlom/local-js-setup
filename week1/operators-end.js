//**JavaScript Operators Extended */

// ++ --> increase a value by 1
var plusPlus = 5;
plusPlus++;
console.log("plusPlus: ", plusPlus);

// -- --> decrease a value by 1
var minusMunus = 10;
minusMunus--;
console.log("minusMunus:", minusMunus);

//**What about ** and // ? */

// **
var timesTimes = 90;
timesTimes;

console.log("timesTimes", timesTimes);

// //
var divideDivide = 45;

//**Operation followed by the "=" operator */
// +=
var plusEquals = 10;
plusEquals += 10;

console.log("plusEquals:", plusEquals);

// -=
var minusEquals = 20;
minusEquals -= 10;

console.log("minusEquals:", minusEquals);

// *=
var timesEquals = 20;
timesEquals *= 10;

console.log("timesEquals:", timesEquals);

// /=
var divideEquals = 100;
divideEquals /= 10;

console.log("divideEquals:", divideEquals);

//Math order of operation: be careful!

var combination = (2 * 2 + 2) / 5;

//what would be the outcome of the above operation?

console.log("combination: ", combination);

//**Operations and strings... be careful!*/
var addString = 5 + 4 + "1"; //will turn the output of first 2 numbers(9) into a string and add it to the string"1" to = 91(string)
console.log("addString:", addString);
console.log("typeof addString:", typeof addString);

//=, ==, ===
console.log(5 == "5");
console.log(5 === "5");

var minusString = "5" - "1";
console.log("minusString:", minusString);
console.log("typeof minusString:", typeof minusString);

var timesString = "5" * "7";
console.log("timesString:", timesString);
// console.log("typeof timesString:", typeof timesString);

var divideString = "16" / "2";
console.log("divideString:", divideString);
// console.log("typeof divideString:", typeof divideString);

let myStr = "this" + " " + "that";
console.log("myStr", myStr);
